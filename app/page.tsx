...
import {
  ...
  LineChart, Line, PieChart, Pie, Cell, AreaChart, Area, ScatterChart, Scatter, ZAxis, ReferenceLine
} from 'recharts';
import {
  AlertTriangle, Download, Filter, Map, ShieldAlert, ShieldCheck, TrendingUp, Users, Activity,
  FileText, CheckCircle, Menu, X, Cpu, Search, Sparkles, Wifi, ArrowRight, Zap
} from 'lucide-react';
          ...
          <div>
            ...
            <div className="space-y-1">
              {[
                ...
                { id: 'map', label: 'Inclusion Map', icon: Map },
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden relative mb-6">
              ...
              <div className="px-6 py-4 flex justify-between items-center">
                <div className="flex items-center gap-4">
                  <button onClick={() => setSidebarOpen(!sidebarOpen)} className="text-slate-500 hover:text-slate-800 transition">
                ...
                <div className="overflow-auto max-h-[350px]">
                  <table className="w-full text-left">
                    <tbody className="divide-y divide-slate-100 text-sm">
                      {(data.inclusion_risks || [
                        { district: "East Khasi Hills", type: "Digital Access Index", volume: "0%" },
                        { district: "Bengaluru Urban", type: "Digital Access Index", volume: "0%" },
                        { district: "Kanpur Nagar", type: "Digital Access Index", volume: "0%" },
                        { district: "Aligarh", type: "Digital Access Index", volume: "0%" },
                      ]).slice(0, 4).map((row: any, i: number) => (
                        <tr key={i} className="group hover:bg-slate-50">
                          ...
                          <td className="p-4 text-right align-top">
                            <div className="text-sm font-bold text-slate-800 mb-1">{row.value ? (row.value * 100).toFixed(0) + "%" : row.volume}</div>
                            <span className="bg-yellow-100 text-yellow-700 text-[9px] font-bold px-1.5 py-0.5 rounded border border-yellow-200 whitespace-nowrap">
                              DIGITALLY DEPENDENT
                            </span>
                          </td>
                        </tr>
        ...
        <div className="col-span-3 flex flex-col justify-between border-r border-green-900/30 pr-4">
          ...
          <div className="flex-1">
            ...
            <div className="space-y-2 mb-4">
              <div className="bg-green-900/10 p-2 border border-green-900/30 rounded">
                {data.anomalies && data.anomalies.length > 0 ? (
                  data.anomalies.slice(0, 5).map((ano: any, i: number) => (
                    <div key={i} className="text-[10px] text-green-400 mb-1 border-b border-green-900/20 pb-1 last:border-0">
                      <span className="text-red-500">⚠ {ano.type}</span>: {ano.district}
                    </div>
                  ))
                ) : (
                  <p className="text-green-900 text-xs italic">No anomalies detected in current dataset.</p>
                )}
              </div>
            ...
            <div className="text-green-800 text-xs">
              {data.anomalies?.length || 0} total anomalies in registry
            </div>
          </div>
          ...
          <div className="border border-green-900/30 bg-black p-4 rounded-lg mt-auto">
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between items-center">
                ...
                <span className="text-red-500 font-bold">{data.anomalies?.filter((a: any) => a.type.includes('Critical')).length || 0}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400">High Priority</span>
                <span className="text-orange-500 font-bold">{data.anomalies?.filter((a: any) => a.type.includes('High')).length || 0}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400">States Affected</span>
                <span className="text-green-500 font-bold">{new Set(data.anomalies?.map((a: any) => a.state)).size || 0}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400">Districts Flagged</span>
                <span className="text-green-500 font-bold">{data.anomalies?.length || 0}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="min-h-[400px]">
          ...
          <div className="w-full h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart layout="vertical" data={ageData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col h-[400px]">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                State-wise Gender Gap
                <span className="bg-pink-100 text-pink-600 text-[10px] px-2 py-0.5 rounded-full uppercase">Real-time</span>
              </h3>
              <p className="text-xs text-slate-500">Female enrolment percentage by state</p>
            </div>
          ...
          <div className="flex-1 w-full relative">
            <div className="absolute inset-0">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.societal_trends?.gender_gap?.slice(0, 10) || []} layout="vertical" margin={{ left: 40, right: 30 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" horizontal={false} />
                  <XAxis type="number" domain={[0, 100]} hide />
                  <YAxis dataKey="state" type="category" tick={{ fontSize: 10, fill: '#64748b', fontWeight: 600 }} width={100} axisLine={false} tickLine={false} />
                  <Tooltip cursor={{ fill: '#fce7f3', opacity: 0.5 }} contentStyle={{ backgroundColor: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                  <Bar dataKey="female_percentage" fill="#ec4899" radius={[0, 4, 4, 0]} barSize={16}>
                    {data.societal_trends?.gender_gap?.slice(0, 10).map((entry: any, index: number) => (
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col h-[400px]">
          ...
          <div className="flex-1 w-full relative">
            <div className="absolute inset-0">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.societal_trends?.age_trends || []} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                  <defs>
                  ...
                  <Area type="monotone" dataKey="new_born_ratio" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorNewBorn)" name="New Borns" />
                  <Area type="monotone" dataKey="adult_ratio" stroke="#a855f7" strokeWidth={3} fillOpacity={1} fill="url(#colorAdult)" name="Adults" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-50 rounded-xl p-6 border border-slate-200 relative overflow-hidden">
          <div className="flex justify-between items-center mb-4 relative z-10">
            <h3 className="font-bold text-slate-800 flex items-center gap-2">
              <Map size={16} /> Migration Hotspots
            </h3>
        ...
        {/* AI ADVISOR */}
        <div className="lg:col-span-2 bg-gradient-to-br from-indigo-900 to-slate-900 rounded-xl p-6 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="bg-slate-50 rounded-xl p-6 border border-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.ai_advisories?.advisories?.map((advice: any, i: number) => (
              <div key={i} className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition">
        ...
        <div className="bg-white rounded-xl p-8 shadow-sm border border-slate-200 overflow-hidden relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-gradient-to-br from-red-50 to-white border border-red-100 p-6 rounded-2xl relative overflow-hidden shadow-sm hover:shadow-md transition group">
              <div className="absolute right-[-6px] top-[-6px] text-red-100/50 group-hover:text-red-200/50 transition-colors transform rotate-12 pointer-events-none">
            ...
            <div className="bg-gradient-to-br from-orange-50 to-white border border-orange-100 p-6 rounded-2xl relative overflow-hidden shadow-sm hover:shadow-md transition group">
              <div className="absolute right-[-6px] top-[-6px] text-orange-100/50 group-hover:text-orange-200/50 transition-colors transform rotate-12 pointer-events-none">
            ...
            <div className="bg-gradient-to-br from-emerald-50 to-white border border-emerald-100 p-6 rounded-2xl relative overflow-hidden shadow-sm hover:shadow-md transition group">
              <div className="absolute right-[-6px] top-[-6px] text-emerald-100/50 group-hover:text-emerald-200/50 transition-colors transform rotate-12 pointer-events-none">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-slate-50 rounded-2xl p-6 border border-slate-200 shadow-innerer-slate-200">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-slate-800">Inclusion-Friction Matrix</h3>
              ...
              <div className="h-[400px] w-full bg-white rounded-xl border border-slate-100 p-2 shadow-inner">
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                    ...
                    <XAxis type="number" dataKey="x" name="Inclusion" stroke="#64748b" tick={{ fontSize: 10 }} label={{ value: 'Demographic Update Ratio (%)', position: 'bottom', offset: 0, fontSize: 10, fill: '#64748b' }} />
                    <YAxis type="number" dataKey="y" name="Friction" stroke="#64748b" tick={{ fontSize: 10 }} label={{ value: 'Biometric Update Ratio (%)', angle: -90, position: 'left', fontSize: 10, fill: '#64748b' }} />
                    ...
                    <div key={i} className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50 hover:border-slate-500 transition group relative overflow-hidden">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-3 pl-2">
                        <div className="bg-slate-900/50 p-2 rounded border border-slate-700/50">
                          <div className="text-[10px] text-slate-500 uppercase">Biometric</div>
                          <div className="text-xs font-bold text-slate-300">{d.y_bio_ratio.toFixed(1)}%</div>
                        </div>
                        <div className="bg-slate-900/50 p-2 rounded border border-slate-700/50">
                          <div className="text-[10px] text-slate-500 uppercase">Demographic</div>
                          <div className="text-xs font-bold text-slate-300">{d.x_demo_ratio.toFixed(1)}%</div>
                        </div>
                      </div>
                      ...
                      <div className="pl-2">
                        <button className={`w-full py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition flex items-center justify-center gap-2 ${d.zone === 'Fraud Risk'
                          ...
                          {d.zone === 'Fraud Risk' ? <><AlertTriangle size={12} /> Init Audit Protocol</> : <><Map size={12} /> Deploy Mobile Unit</>}
                        </button>
                      </div>
                    </div>
                  ))}
